...
const API_BASE_URL = 'https://gapitest.yban.co';
...
export default function LoginScreen() {
  ...
  const handleGetVerificationCode = async () => {
    ...
    try {
      setLoading(true);
      const response = await fetch(`${API_BASE_URL}/GovService_LoginSendCode`, {
        method: 'POST',
        headers: {
      ...
      if (data.Code === 0) {
        ...
        safeError('Failed to send code:', data.Message);
        Alert.alert('错误', data.Message || '发送验证码失败，请重试');
      }
    } catch (error) {
      safeError('Error sending verification code:', error);
  ...
  const handleLogin = async () => {
    ...
    try {
      ...
      const response = await fetch(`${API_BASE_URL}/GovService_Login`, {
        method: 'POST',
        headers: {
          'Content-Type': 'application/json',
        },
      ...
      if (data.Code === 0 && data.Token) {
        ...
        Alert.alert('错误', data.Message || '登录失败，请重试');
      }
    } catch (error) {
      Alert.alert('错误', '网络错误，请稍后重试');
    } finally {
